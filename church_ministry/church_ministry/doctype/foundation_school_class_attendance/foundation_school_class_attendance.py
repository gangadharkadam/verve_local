...
def validate_duplicate(doc,method):
	if doc.get("__islocal"):
		res=frappe.db.sql("select name from `tabFoundation School Class Attendance` where  church='%s' and date='%s' and docstatus!=2"%(doc.church,doc.date))
		if res:
			frappe.throw(_("Another Foundation School Class Attendance '{0}' Cell Code '{2}' and date '{3}' exist..!").format(res[0][0],doc.church,doc.date))
	today=nowdate()
