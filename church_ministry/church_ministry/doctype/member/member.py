...
class Member(Document):
	...
	def on_update(self):
		...
		u_rle=frappe.db.sql("select name from `tabUserRole` where parent='%s' and role='First Time Visitor'"%(self.email_id),as_list=1)
		# frappe.errprint(u_rle)
		if self.flag=='not':
			if usr_id:
				if u_rle:
					frappe.db.sql("delete from `tabUserRole` where parent='%s' and role='First Time Visitor'"%(self.email_id))
				...
				if self.member_designation=='Region Pastor':
					...
					r_user = 'Member'
					perm = 'Member'
				elif self.member_designation=='FT Visitor':
					c_user = u_rle[0][0]
					r_user = 'First Time Visitor'
					perm = 'First Time Visitor'
				# frappe.errprint("create user")
